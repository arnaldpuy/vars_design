...
## ----design_figure, cache=TRUE, dependson="functions", warning=FALSE---------------
...
full.plot <- plot_grid(legend, bottom, ncol = 1, rel_heights = c(0.2, 1))


## ----plot_fig1, cache=TRUE, dependson="design_figure", fig.width=4.5, fig.height=4.5, warning=FALSE----
...
## ----plot_fig_2, cache=TRUE, fig.width=5.5, fig.height=5, warning=FALSE, dependson="functions"----
...
## ----plot_figures_separately, cache=TRUE, dependson=c("plot_fig_2", "design_figure", "plot_figure_1"), fig.height=2, fig.width=4----
...
## ----plot_figures_separately2, cache=TRUE, dependson=c("plot_fig_2", "design_figure", "plot_figure_1"), fig.height=2, fig.width=5----
...
## ----plot_fig_liu, cache=TRUE, fig.width=3, fig.height=4.5-------------------------
...
## ----liu_tikz, dev="tikz", cache=TRUE, echo=FALSE, fig.width=5.4, fig.height=2.2----
...
model_ti <- function(N.stars, h, k, k_2, k_3, epsilon, delta, tau, phi, N.jansen, N.high) {
  ...
  ind.vars <- vars_ti(output[lg.vars], star.centers = N.stars, 
    ...
    .[, sample.size:= "n"]
  full.vars <- rbind(ind.vars, full.ind)[, estimator:= "VARS"]
  # Compute Jansen
...
full_output <- merge(mt.dt, out_cor) %>%
  .[, Nt:= ifelse(estimator == "VARS", Nt.vars, Nt.jansen)]
